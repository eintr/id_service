...
int syslog_set_target(enum log_target_en, ...); 
int syslog_clear_target(enum log_target_en);
...
#define syslog(level_, format_, ...) do{if(level_<=log_base_level_){snprintf(log_buffer_, LOG_BUFFER_SIZE, "%s/%s(%d): %s", __FILE__, __FUNCTION__, __LINE__, format_);do_syslog(level_, log_buffer_, ##__VA_ARGS__);}}while(0)
#else
#define syslog(level_, format_, ...) do{if(level_<=log_base_level_){do_syslog(level_, format_, ##__VA_ARGS__);}}while(0)
#endif
